...
tokenstream_class = stream_class:inherit()

function tokenstream_class:from_charstream (charstream)
   return self:new { charstream = charstream }
end

token_grammar = << comments = repeat spaces_and_comments, token = token 
                :: add_comments (comments, token) >>

spaces_and_comments = << repeat (spaces, comment=comment :: comment) >>

spaces = << >>

comment = << >>

token = << a=letter, b=repeat (letter || number) 
        :: build_word (a .. table.concat(b)) >>

build_word = |w| build_token (alpha_keyword[w] and "Keyword" or "Id", w)



function tokenstream_class.methods:extract()
...
function tokenstream_class.methods:dup()
